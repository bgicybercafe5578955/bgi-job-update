...
function statusPill(status: string) {
  ...
  return map[status || "published"] || "bg-ink-900/5 text-ink-700";
}
...
export default async function ManageJobsPage({
  ...
  try {
    ...
    const jobs = rawJobs.map((job: any) => ({
      ...
      views: job.views || 0,
      dates: { lastDate: job.dates?.lastDate ? new Date(job.dates.lastDate) : new Date() },
              ...
              {jobs.map((job) => (
                <tr key={job.id} className="border-b border-ink-900/5 last:border-0">
                  ...
                  <td className="px-4 py-3 text-ink-700 whitespace-nowrap">
                    {job.dates.lastDate instanceof Date && !isNaN(job.dates.lastDate.getTime())
                      ? job.dates.lastDate.toLocaleDateString("en-IN", { day: "2-digit", month: "short", year: "numeric" })
                      : "N/A"}
                  </td>
    ...
    return (
      <div className="p-6 bg-red-50 border border-red-200 rounded-xl text-red-700">
        <h2 className="text-lg font-bold">Error Loading Manage Jobs</h2>
